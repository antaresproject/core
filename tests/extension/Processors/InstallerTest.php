...
class InstalledTest extends OperationSetupTestCase
{
    ...
    public function getOperationProcessor() {
        return new Installer($this->composerHandler, $this->extensionValidator, $this->container, $this->dispatcher, $this->kernel, $this->extensionsRepository, $this->settingsFactory);
    }
    ...
    public function testWithoutComposerAsSuccess() {
        ...
        $this->assetManager->shouldReceive('extension')->once()->with($name)->andReturnNull()->getMock();
    ...
    public function testWitComposerAsSuccess() {
        $processor = $this->getOperationProcessor();
        ...
        $this->composerHandler->shouldReceive('run')
            ->with('composer require ' . $name, function() {})
            ->andReturn($process)
            ->getMock();
