...
$(document).ready(function () {
    ...
    $('.header-translations').on("change", '.column-translation', function (e) {
        ...
        $.ajax({
            ...
            success: function (response) {
                $('.translations-container tbody tr').each(function (index, item) {
                    var key = $(item).find('td:nth-child(2) .table-key__text').text().trim();
                    for (var i = 0; i < response.length; i++) {
                        if (response[i].key === key) {
                            $(item).find('td:nth-child(4) .table-key__inner span').html(response[i].value);
                        }
                    }
